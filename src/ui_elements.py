...
class Inbox(UIElement):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        if self.input or not self.placeholder_active:
            ...
            )  # this way for passward will see *
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        if event.type == pygame.KEYDOWN:  # Обработваме само KEYDOWN събития
            if self.active:  # Ако полето е активно
                if event.key == pygame.K_RETURN:  # Enter - Потвърждаване
                    self.last_input = self.input
                    self.input = ""
                    self.placeholder_active = True
                elif event.key == pygame.K_BACKSPACE:  # Backspace - Изтриване
    ...
    def update(self) -> None:
        # Деактивиране на placeholder, ако текстът е въведен
