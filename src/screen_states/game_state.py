...
class GameState(State):
    def __init__(
        ...
        self.loser_color = None  # when game is over to know the outcome for ranglist
    ...
    def _remove_invalid_moves(self, figure: Figure) -> None:
        figure.moves = list(
            filter(
                lambda move: self.board.squares[move.final.row][
                    move.final.col
                ].is_empty()
                or self.board.squares[move.final.row][
                    move.final.col
    ...
    def _handle_event_mouse_button_down(self, event: pygame.event.Event) -> None:
        ...
        if self.button_to_surrender.is_clicked(event.pos):
            ...
            self.end_player_vs_player_game()
        else:
    ...
    def _hande_ai_move(self) -> None:
        self.set_game_over(Colors.WHITE)  # check if player win
        if self.is_make_moved:
            # first if player won the game
            if self.is_game_over:
                if State.is_load_ranglist:
                    if not self.loser_color:
                        State.ranglist.update_elo(
                ...
                self.gen_ai_move()
                self.set_game_over(Colors.BLACK)
                # check if ai win
                if self.is_game_over:
                    if State.is_load_ranglist:
                        if not self.loser_color:
    ...
    def end_player_vs_player_game(self):
        # if it's PLAYER_VS_PLAYER only here check for game_over
        if self.is_game_over and self.loser_color:
            if State.is_load_ranglist:
                outcome = 1 if self.white_player.color != self.loser_color else 0
                State.ranglist.update_elo(
                    self.white_player.user, self.black_player.user, outcome
                )
                State.current_state = ScreenState.FINAL
            else:
                State.current_state = ScreenState.FINAL
        elif self.is_game_over and not self.loser_color:
            if State.is_load_ranglist:
                State.ranglist.update_elo(
                    self.white_player.user, self.black_player.user, 1 / 2
                )
