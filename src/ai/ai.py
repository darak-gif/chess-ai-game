...
class AI:
    ...
    def minimax(
        self, depth: int, alpha, beta, is_ai_turn: bool
    ) -> tuple[Move | None, float]:
        board_hash = self.hasher.hash_board(self.curr_board)
        ...
        if is_ai_turn:
            ...
            for move in self.all_posible_moves(self.color):
                ...
                captured_fig = copy.deepcopy(
                    self.curr_board.squares[move.final.row][move.final.col].figure
                )
                ...
                self.curr_board.undo_move(fig, captured_fig, move)
